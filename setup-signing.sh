#!/bin/bash
# ──────────────────────────────────────────────────────────────
# setup-signing.sh — Create a persistent self-signed code-signing
# certificate for Pastry development.
#
# This certificate lives in your login keychain and gives every
# dev build a STABLE identity so macOS TCC (Accessibility, etc.)
# remembers permissions across rebuilds.
#
# Run this ONCE. It is safe to re-run — it skips if the
...
# Trust for code signing (may require admin password)
