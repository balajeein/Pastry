...
build_debug() {
    ...
    xcrun swiftc -swift-version 5 -target arm64-apple-macos11.0 -g -Onone \
        -F "$PROJECT_DIR/Frameworks" -framework Sparkle \
        -Xlinker -rpath -Xlinker @executable_path/../Frameworks \
        -o "$BINARY-arm64" $(find "$PROJECT_DIR/Pastry" "$PROJECT_DIR/PastryApp" -name "*.swift")

    xcrun swiftc -swift-version 5 -target x86_64-apple-macos11.0 -g -Onone \
        -F "$PROJECT_DIR/Frameworks" -framework Sparkle \
        -Xlinker -rpath -Xlinker @executable_path/../Frameworks \
        -o "$BINARY-x86_64" $(find "$PROJECT_DIR/Pastry" "$PROJECT_DIR/PastryApp" -name "*.swift")
