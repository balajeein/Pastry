...
public class PastryPanelController: NSObject, NSWindowDelegate {
    ...
    public func showPanel() {
        ...
        // Capture selected text from the previous app (simulates ⌘C, reads, restores clipboard)
        // Must happen while the previous app still has focus
        PasteService.shared.captureSelectedText()
        
        // Reset search field and selection index every time the panel opens
        viewModel.searchText = ""
        viewModel.selectedIndex = 0
        
        // Detect if the selected text is a math expression
        viewModel.detectCalculation()
